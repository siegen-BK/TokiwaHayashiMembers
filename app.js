(() => {
  ...
  function updateStickyLayout() {
    ...
    root.setProperty('--sticky-top-toolbar', `${h}px`);

    if (toolbar) {
      const t = toolbar.offsetHeight || 0;
      root.setProperty('--toolbar-h', `${t}px`);
      root.setProperty('--sticky-top-tablehead', `${h + t}px`);
    } else {
      root.setProperty('--toolbar-h', `0px`);
      root.setProperty('--sticky-top-tablehead', `${h}px`);
    }
  }
  ...
  function serializeRows() {
    const rowsEl = $('#rows');
    if (!rowsEl) return [];
    return $$('.row-group', rowsEl).map(group => {
      const obj = {};
  ...
  function initRouting() {
    if (typeof window.route !== 'function' || typeof window.navigate !== 'function') {
      const v = $('#view');
      if (v) v.textContent = 'router.js の読み込みに失敗しました';
      return;
    }
    ...
    window.route('/404',     () => { const v = $('#view'); if (v) v.textContent = '404'; });
  ...
  function initEvents() {
    ...
    $('#view').addEventListener('keydown', (e) => {
      ...
      e.preventDefault();
      const list = $$('#rows .cell[contenteditable="true"]');
